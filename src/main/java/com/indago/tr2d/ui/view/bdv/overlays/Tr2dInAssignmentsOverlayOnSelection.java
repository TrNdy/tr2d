 ...
 * @author jug
 */
public class Tr2dInAssignmentsOverlayOnSelection extends BdvOverlay {
				...
				if ( showMovements ) {
					...
					for ( final MovementHypothesis move : segvar.getInAssignments().getMoves() ) {
						if ( pgSolution.getAssignment( move ) == 1 ) {
							drawMove( g, trans, move, listActiveColor, i++, segvar.getInAssignments().getMoves().size() );
						} else {
							drawMove( g, trans, move, listMovementColors, i++, segvar.getInAssignments().getMoves().size() );
						}
					}
				}
				if ( showDivisions ) {
					...
					for ( final DivisionHypothesis div : segvar.getInAssignments().getDivisions() ) {
						if ( pgSolution.getAssignment( div ) == 1 ) {
							drawDivision( g, trans, div, listActiveColor, i++, segvar.getInAssignments().getDivisions().size() );
						} else {
							drawDivision( g, trans, div, listDivisionColors, i++, segvar.getInAssignments().getDivisions().size() );
						}
					}
				}
			}
	...
	private void drawMove(
		...
		g.setColor( Color.lightGray ); // shadow
		final String str = String.format( " c=%.2f", move.getCost() );
		g.drawString(
				str,
				( int ) ( gposFrom[ 0 ] - metrics.getStringBounds( str, g ).getWidth() - 1 ),
...
//		g.drawLine( ( int ) gposFrom[ 0 ] - 1, ( int ) gposFrom[ 1 ] + 1, ( int ) gposTo[ 0 ] - 1, ( int ) gposTo[ 1 ] + 1 );
	...
	private void drawDivision(
		...
		final FontMetrics metrics = g2.getFontMetrics( font );
		g.setColor( Color.lightGray ); // shadow
		final String str = String.format( " c=%.2f", division.getCost() );
		g.drawString(
				str,
				( int ) ( gposFrom[ 0 ] - metrics.getStringBounds( str, g ).getWidth() - 1 ),
				( int ) ( gposFrom[ 1 ] + metrics.getHeight() * ( i + 0.5 ) + 1 ) );
